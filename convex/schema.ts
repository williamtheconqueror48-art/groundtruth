...
export default defineSchema({
  ...
  // LEGACY (2026-09-23 strip): the plan-limit notice backend
  // (convex/apiPlanLimitNotices.ts) was removed with the commercial subsystem.
  // The table stays in the schema so account deletion can purge legacy rows;
  ...
  // LEGACY (2026-09-23 strip): partner-embed keys (`wme_…`) and the embed-key
  // backend were removed with the commercial subsystem. The table stays in
  // the schema so account deletion can purge legacy rows; no live code reads
  // or writes it.
  ...
  // LEGACY (2026-09-23 strip): Pro MCP tokens were removed with the
  // commercial subsystem. The table stays in the schema so account deletion
  // can purge legacy rows; no live code reads or writes it.
